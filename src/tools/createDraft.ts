 ...
 *
 * Logic Overview:
 * 1. Defines a Zod schema (`createDraftSchema`) to validate the parameters for creating a draft.
 * 2. The `createDraftHandler` function takes the validated parameters.
 * 3. It constructs a `backlog draft create` CLI command based on the provided parameters.
...
/**
 * Defines the schema for the parameters required to create a draft.
...
/**
 * Handles the logic for creating a draft.
 * It constructs and executes the `backlog draft create` command.
 *
 * @param params The input parameters, validated against the `createDraftSchema`.
...
export async function createDraftHandler(
  ...
  try {
    ...
    return {
      content: [
        { type: 'text', text: `Draft created successfully: ${stdout.trim()}` },
      ],
    };
  } catch (error: unknown) {
