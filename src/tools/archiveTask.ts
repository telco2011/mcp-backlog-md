 ...
 *
 * Logic Overview:
 * 1. Defines a Zod schema (`archiveTaskSchema`) to validate the parameters for archiving a task.
 * 2. The `archiveTaskHandler` function takes the validated parameters.
 * 3. It constructs a `backlog task archive` CLI command based on the provided parameters.
...
/**
 * Defines the schema for the parameters required to archive a task.
...
/**
 * Handles the logic for archiving a task.
 * It constructs and executes the `backlog task archive` command.
 *
 * @param params The input parameters, validated against the `archiveTaskSchema`.
 * @returns An object containing the result of the command execution.
 */
export async function archiveTaskHandler(
  params: z.infer<typeof archiveTaskSchema>
) {
  ...
  try {
    ...
    return {
      content: [
        { type: 'text', text: `Task archived successfully: ${stdout.trim()}` },
      ],
    };
  } catch (error: unknown) {
